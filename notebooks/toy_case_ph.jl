...
function J(x)
	J = zeros(3,3)
	J[1,1] = 10*x[3]*cos(x[1]) + 10*x[3]*cos(x[1] - x[2])
	J[1,2] = -10*x[3]*cos(x[1] - x[2]) 
	J[1,3] = 10*sin(x[1]) + 10*sin(x[1] - x[2])
	J[2,1] = -10*x[3]*cos(x[2] - x[1])
	J[2,2] = 10*cos(x[2]) + 10*x[3]*cos(x[2] - x[1])
	J[2,3] =  10*sin(x[2] - x[1])
	J[3,1] = 10*x[3]*sin(x[1]) + 10*x[3]*sin(x[1] - x[2])
	J[3,2] = -10*x[3]*sin(x[1] - x[2])
	J[3,3] =  -10*cos(x[1])+40*x[3]-10*cos(x[1]-x[2])
	return J
end
...
# ╔═╡ 907f617c-a51c-44d8-b3ae-44faaeebea63
"""
Newton raphson update.
"""
function nr_update(x_k)
	return x_k - inv(J(x_k))*f(x_k)
end
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "e08b705584077f3ccbab83e94f30334f46189e5e"
