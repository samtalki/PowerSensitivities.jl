...
Params:
pd - MxN matrix of demanded active power
qd - MxN matrix of demanded reactive power
vm - MxN matrix of bus voltage magnitudes
lambd - ℓ2 regularization
"""
function calc_vm_sens(pd,qd,vm)
	m,n = size(pd)
	spv = inv(pd'*pd + lambd*eye(n))*pd'*vm
	sqv = inv(qd'*qd + lambd*eye(n))*qd'*vm
	return spv,sqv
end
...
lambd - ℓ2 regularization
"""
function calc_pq_sens(pd,qd,vm)
	m,n = size(pd)
	svp = inv(vm'*vm + lambd*eye(n))*vm'*pd
	svq = inv(vm'*vm + lambd*eye(n))*vm'*qd
	return svp,svq
end
