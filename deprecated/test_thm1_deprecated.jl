...
"""
function calc_rhs(network::Dict{String,<:Any},sel_bus_types,drop_bad_idx=true)
...
If drop_bad_idx==True, then the buses that meet the conditions in calc_bad_idx are discarded.
"""
function calc_lhs(network::Dict{String,<:Any},sel_bus_types,drop_bad_idx=true)
...
"""
function test_rhs(sel_bus_types,network_data_path=network_data_path)
...
Test LHS for all feeder models in network_data_path
"""
function test_lhs(sel_bus_types,network_data_path=network_data_path)
    names = readdir(network_data_path);
    paths = readdir(network_data_path,join=true);
    results = Dict()
    for (name,path) in zip(names,paths)
...
"""
function test_thm1(sel_bus_types,network_data_path=network_data_path)
    names = readdir(network_data_path);
    paths = readdir(network_data_path,join=true);
    results = Dict()
    for (name,path) in zip(names,paths)
